...
"additionalneeds" : "wi-fi"
        ...
        //4-Assertion
        JsonPath responseJsonPath=response.jsonPath(); // bu method bizim response'ımızı jsonpath ile kullanılabileck
        //bir hale getirmis oluyor.

        //her zaman ilk yazılan expected -------> olusturdugumuz JsonObj:expected data
        // sonraki actual------------>response: responseJsonPath
            ...
            assertEquals(expectedData.getJSONObject("booking").get("firstname")
                    ,responseJsonPath.get("booking.firstname"));
            assertEquals(expectedData.getJSONObject("booking").getJSONObject("bookingdates")
                            .get("checkin")
                    ,responseJsonPath.get("booking.bookingdates.checkin"));
            assertEquals(expectedData.getJSONObject("booking").getJSONObject("bookingdates")
                    .get("checkout"),responseJsonPath.get("booking.bookingdates.checkout"));
